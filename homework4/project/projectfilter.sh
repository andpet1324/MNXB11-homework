...
########################################################################
# 
# shmifilter.sh - A filter script that uses projectcleaner.sh bare data
#
# Author: Florido Paganelli florido.paganelli@fysik.lu.se and Andreas Pettersson, Ellen Brandt Sahlberg and Verena Hehl.
#
# Description: this script manuipulates a baredata dataset generated by 
#              projectcleaner.sh to apply some filters.
#              projectcleaner.sh needs to be in the same folder as this script
#              for this to work.
#
# Examples:
...
# Exit immediately if the projectcleaner.sh script is not found
if [ ! -f 'projectcleaner.sh' ]; then
...
log "Calling projectcleaner.sh script"
./projectcleaner.sh $FILTER_SMHIINPUT

if [[ $? != 0 ]]; then
   echo "projectcleaner.sh failed, exiting..." 1>&2
   exit 1
fi

# projectcleaner.sh generates a filename that starts with baredata_<datafilename>
# So storing it in a variable for convenience.
CLEANER_BAREDATAFILENAME="baredata_$FILTER_DATAFILE"

log "Begin filtering..."

echo ${FILTER_SMHIINPUT}
